...
const BUFFER16 = Buffer.alloc(16, 0x00);
...
binet.encode = function encode(raw) {
  ...
  if (raw.length === 16) {
    ...
    if (inet.mapped(raw, 0))
      str = inet.ntop4(raw, 12);
    else
      str = inet.ntop6(raw, 0);
...
binet.toHost = function toHost(host, port, key) {
  ...
  const type = binet.getStringType(host);
...
binet.write = inet.write;
binet.family = inet.family;

binet.ntop4 = inet.ntop4;
binet.ntop6 = inet.ntop6;
binet.ntop = inet.ntop;
binet.read = inet.read;

binet.mapped = inet.mapped;
binet.normal = inet.normal;
