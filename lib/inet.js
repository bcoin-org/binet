...
function pton(src, dst, off) {
  ...
  const bits4 = pton4(src, dst, off);

  if (bits4 >= 0)
    return [4, bits4];

  const bits6 = pton6(src, dst, off);
...
function write(src, dst, off) {
  const [af] = pton(src, dst, off);
...
function read(src, off, len) {
  const [af, str] = ntop(src, off, len);
...
function normal(str) {
  const dst = BUFFER16;
  const af = write(str, dst, 0);

  switch (af) {
    case 4:
      return read(dst, 0, 4);
    case 6:
      return read(dst, 0, 16);
    default:
      throw new Error('Invalid IP address.');
  }
}
